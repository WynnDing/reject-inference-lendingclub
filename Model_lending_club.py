...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug  7 11:36:48 2019
...
import numpy as np
import os
os.chdir(r'/Users/wynn/Desktop/HandsomeBike/rmpgy')
import warnings
...
def time_convert(timeStr):
    if str(timeStr).lower() == 'nan':
        ...
        time_new = time_split[1] + '-' + month_name_dict[time_split[0]]
        return time_new
    
## 数据切分
#def data_split(arr):
#    if str(arr) in ['Dec-2012']:#,'Nov-2012'
#        return 'test'
#    elif str(arr) in ['Nov-2012']:#'Sep-2012','Oct-2012'
#        return 'valid'
#    else:
#        return 'train'
#
## 时间切分
#def time_split(arr):
#    for time in ['2010','2011','2012']:#'2009','2010',
#        if time in str(arr):
#            return 1
#    return 0
...
@timecount()
def load_data():
    train_data = pd.read_csv(r'/Users/wynn/Desktop/HandsomeBike/accepted_2007_to_2018Q4.csv',\
                         header = 0, encoding = 'utf-8',na_values = '\\N',parse_dates=[1])
    reject_data = pd.read_csv(r'/Users/wynn/Desktop/HandsomeBike/rejected_2007_to_2018Q4.csv',\
                        header = 0, encoding = 'utf-8',na_values = '\\N',parse_dates=[1])
    ...
    reject_data_temp['Debt-To-Income Ratio'] = reject_data_temp['Debt-To-Income Ratio'].map(lambda x:float(x.split('%')[0]))
    ...
    data_temp = train_data_temp[(train_data_temp['Application Date']>'2011-04')&\
                                 (train_data_temp['Application Date']<='2011-08')] 
    ...
    train_data_new = train_data[feature+['user_type']]
    valid_data_new = valid_data[feature+['user_type']]
    test_data_new = test_data[feature+['user_type']]
    reject_data_new = reject_data_temp[feature]
    return train_data_new,reject_data_new,valid_data_new,test_data_new
...
def model_generate(param_dict,types='train'):
    ...
    if types == 'train':
        ...
        model_list = [lightgbm_model,xgboost_model,catboost_model]
    else:
        raise ValueError
    return model_list
...
def reject_main_train(reject_train,train_data,predict_reject_prob,model_list,\
    ...
    for ratio in [0.5,1,2]:
        for n_sample in [1000,2000]:   #,4000 
...
# 贝叶斯调参
from pgy_model import bayes_parameter_opt_lgb,bayes_parameter_opt_xgb
...
def result_save(result,name):
    writer = pd.ExcelWriter(r'/Users/wynn/Desktop/HandsomeBike/result_1_轮_2.xlsx',engine='xlsxwriter')
...
reject_evalutaion_detail,pipeline_model_userful,reject_train_userful = reject_main_train(reject_train,\
                                                train_data,predict_reject_prob,model_list_eval,feature_used,model_auc_raw,0.65,0.25,diff=0)
...
result_save(temp_1,name=u'0.62_0.25_深度3_时间2011-2013_0.63')

# 保存第一轮最佳的数据集与模型
pipeline_model_reject_1 = pipeline_model_userful[1]
userful_reject_data_1 = reject_train_userful[1]
from sklearn.externals import joblib
joblib.dump(pipeline_model_reject_1,r'/Users/wynn/Desktop/HandsomeBike/best_model_1_0.pkl',compress=3)
userful_reject_data_1.to_csv(r'/Users/wynn/Desktop/HandsomeBike/best_data_1_0.csv',index=False)


# 开始第二轮迭代: 输出最佳结果对应的model
model_auc_reject_1 = 0.62
reject_data_temp['prob_1'] = predict_reject_prob
reject_data_temp_2 = reject_data_temp[(reject_data_temp['prob_1']<0.65)&(reject_data_temp['prob_1']>0.25)] 
...
reject_evalutaion_detail_2,pipeline_model_userful_2,reject_train_userful_2 = reject_main_train(reject_train_2,\
                                                train_data,predict_reject_prob_2,model_list_eval,feature_used,model_auc_reject_1,0.8,0.3,diff=0)
...
from sklearn.externals import joblib
joblib.dump(pipeline_model_reject_2,r'/Users/wynn/Desktop/HandsomeBike/best_model_2.pkl',compress=3)
userful_reject_data_2.to_csv(r'/Users/wynn/Desktop/HandsomeBike/best_data_2.csv',index=False)
...
from sklearn.externals import joblib
joblib.dump(pipeline_model_reject_3,r'/Users/wynn/Desktop/HandsomeBike/best_model_3_0.7437.pkl',compress=3)
userful_reject_data_3.to_csv(r'/Users/wynn/Desktop/HandsomeBike/best_data_3_0.7437.csv',index=False)
# 第二轮结果输出:
# 阈值0.85_0.15_深度4_时间0913-1115_正则_2 0.744730 
# 阈值0.85_0.15_深度4_时间1001-1115_正则_2 0.745252
# 阈值0.85_0.15_深度4_时间1015-1115_正则_2 
eval_reject_df_2 = pd.DataFrame(data = reject_evalutaion_detail_2,index=['valid_ks','valid_auc',\
                                          'ks_test','auc_test','auc_train','ks_train','accuracy'])
temp_2 = eval_reject_df_2.merge(
eval_df,on=[eval_reject_df_2.index],how='inner')
temp_2
#result_save(temp_2,name = u'阈值0.85_0.15_深度4_时间0913-1115_正则_2', 'test_reject_time')
#result_save(temp_2,name = u'阈值0.85_0.15_深度4_时间1001-1115_正则_2', 'test_reject_time')
